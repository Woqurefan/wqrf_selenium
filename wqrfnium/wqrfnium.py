...
print 'Your elements.xls path is %s'%elements_xls_path
...
if not os.path.exists(elements_xls_path):
    ...
    print 'You can also edit wqrfnium.py,find the elements_xls_path,and then correct the path!'
    print "First cell is element's icon,second is element's tmp_find_method,third is element's tmp_find_value,forth is element's index,the last is element's html_element"
...
def getelement(driver,icon):
    ...
    try:
        ...
        print 'success! %s s'%str(time.time()-time1)[:5]
        return el
    except:
        old_html = element[3]
        print 'find_fali,begin fix'
        ...
        print 'find success:%s s'%str(time.time()-time1)[:5],
        return newel
